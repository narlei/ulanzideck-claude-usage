...
// Windows: read OAuth token from ~/.claude/.credentials.json
// Structure mirrors the macOS Keychain entry: { claudeAiOauth: { accessToken: "..." } }
...
// On Windows, try to find claude.exe to trigger a token refresh
const CLAUDE_CANDIDATES = [
  'claude',
  path.join(os.homedir(), 'AppData', 'Roaming', 'Claude', 'claude-code', '2.1.187', 'claude.exe'),
  path.join(os.homedir(), 'AppData', 'Local', 'Claude', 'claude.exe'),
];

function spawnClaude(bin) {
  return new Promise((resolve) => {
    // shell:true is required on Windows to resolve `claude.cmd`/`claude.ps1` shims.
    // Args are hard-coded constants (no user input), so there is no injection risk.
...
export async function fetchUsage({ signal, _retried, force } = {}) {
  ...
  if (!token) {
    return { ok: false, kind: ErrorKind.NO_TOKEN, message: 'No Claude Code credentials found in ~/.claude/.credentials.json' };
  }
